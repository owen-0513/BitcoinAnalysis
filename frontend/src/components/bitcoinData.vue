<template>
  <div class="crypto-data">
    ...
    <div class="tabs">
      ...
      <button @click="updateTab('btc_predict')" :class="{ active: activeTab === 'btc_predict' }">比特幣預測</button>
...
export default {
  ...
  methods: {
    async fetchCryptocurrencyData() {
      try {
        const response = await axios.get('https://bitcoinanalysis.onrender.com/cryptocurrency');
    ...
    async fetchPredictedPrices() {
      try {
        const btcResponse = await axios.get('https://bitcoinanalysis.onrender.com/predict');
        this.predictedBtcPrice = btcResponse.data.predicted_price;
        this.recommendations = btcResponse.data.recommendations;

        const ethResponse = await axios.get('https://bitcoinanalysis.onrender.com/predict_eth');
